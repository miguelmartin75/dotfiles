...
(after! org
    (setq org-capture-templates
        ...
        "* TODO %?\n:LOGBOOK:\n- State \"TODO\" from  %U\n:END:" :empty-lines 1)
        ("B" "Work Backlog" entry (file "~/org/backlog.org")
        "* TODO %? :work:\n:LOGBOOK:\n- State \"TODO\" from  %U\n:END:" :empty-lines 1)
        ("t" "Todo" entry (file+olp+datetree "~/org/journal.org")
        "* TODO %?\n:LOGBOOK:\n- State \"TODO\" from  %U\n:END:" :empty-lines 1)
        ("T" "Work Todo" entry (file+olp+datetree "~/org/journal.org")
        "* TODO %? :work:\n:LOGBOOK:\n- State \"TODO\" from  %U\n:END:" :empty-lines 1)
        ("l" "Log" entry (file+olp+datetree "~/org/journal.org")
        "* %U %?" :empty-lines 1)
        ("L" "Work Log" entry (file+olp+datetree "~/org/journal.org")
        "* %U %? :work:" :empty-lines 1)
        ("i" "Idea" entry (file+olp "~/org/life.org" "Idea")
...
(after! lsp-mode
  ...
  (setq +lsp-company-backends '((:seperate company-yasnippet company-capf)))
  ...
  ;;   (progn
  ;;     (message "yo company")
  ;;     (setq company-backends '((:seperate company-yasnippet company-capf)))))
  ;; )
)
...
(after! company
  ...
  (add-hook 'objc-mode-hook (lambda () (progn (setq company-backends '((:separate company-irony-c-headers company-irony))) (message "objc hook triggered"))))
  ;;(add-variable-watcher 'company-backends (lambda (symbol newval op where) (message "company-backend changed s=%s, val=%s, op=%s, where=%s" symbol newval op where)))
)
#+end_src
