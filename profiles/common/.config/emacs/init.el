...
(defun my/markdown-swap-paragraphs (up)
  "Swap the paragraph at point with its adjacent Markdown paragraph.
When UP is non-nil, swap with the preceding paragraph."
  (let (current-start current-end adjacent-start adjacent-end)
    (cond
     ((eq major-mode 'markdown-ts-mode)
      (let* ((node (treesit-node-at (point) 'markdown))
             (paragraph (and node
                             (treesit-parent-until node "\\`paragraph\\'" t)))
             (adjacent (and paragraph
                            (if up
                                (treesit-node-prev-sibling paragraph t)
                              (treesit-node-next-sibling paragraph t)))))
        (unless (and paragraph
                     adjacent
                     (equal (treesit-node-type adjacent) "paragraph"))
          (user-error "No adjacent Markdown paragraph"))
        (setq current-start (treesit-node-start paragraph)
              current-end (treesit-node-end paragraph)
              adjacent-start (treesit-node-start adjacent)
              adjacent-end (treesit-node-end adjacent))))
     ((derived-mode-p 'markdown-mode)
      (let (current-bounds adjacent-bounds current-heading adjacent-heading invalid)
        (save-excursion
          (when (or (markdown-code-block-at-point-p)
                    (looking-at-p (concat "[ \t]*" markdown-regex-html-tag))
                    (markdown-line-is-reference-definition-p)
                    (get-text-property (line-beginning-position)
                                       'markdown-blockquote)
                    (get-text-property (line-beginning-position) 'markdown-hr)
                    (get-text-property (line-beginning-position)
                    ...
                    (get-text-property (line-beginning-position)
                                       'markdown-metadata-key)
                    (get-text-property (line-beginning-position)
                                       'markdown-metadata-value)
                    (get-text-property (line-beginning-position)
                                       'markdown-metadata-markup))
            (user-error "Not in an ordinary Markdown paragraph"))
          (setq current-bounds (markdown-bounds-of-thing-at-point 'paragraph))
          (unless current-bounds
            (user-error "Not in an ordinary Markdown paragraph"))
          (setq current-start (car current-bounds)
                current-end (cdr current-bounds))
          (goto-char current-start)
          (while (and (< (point) current-end)
                      (looking-at-p "[ \t]*$"))
            (forward-line 1))
          (setq current-start (point))
          (goto-char current-end)
          (when (and (bolp) (> (point) current-start))
            (forward-line -1))
          (while (and (looking-at-p "[ \t]*$")
                      (> (point) current-start))
            (forward-line -1))
          (setq current-end (line-end-position))
          (when (>= current-start current-end)
            (user-error "Not in an ordinary Markdown paragraph"))
          (goto-char current-start)
          (when (or (markdown-heading-at-point)
                    (markdown-list-item-at-point-p)
          ...
          (setq current-heading
                (when (markdown-back-to-heading-over-code-block nil t)
                  (point)))
          (goto-char (if up current-start current-end))
          (if up
              (progn
            ...
            (when (looking-at-p "[ \t]*$")
              (user-error "No adjacent Markdown paragraph")))
          (setq adjacent-start (point)
                adjacent-bounds (markdown-bounds-of-thing-at-point 'paragraph))
          (unless adjacent-bounds
            (user-error "No adjacent Markdown paragraph"))
          (setq adjacent-start (car adjacent-bounds)
                adjacent-end (cdr adjacent-bounds))
          (goto-char adjacent-start)
          (while (and (< (point) adjacent-end)
                      (looking-at-p "[ \t]*$"))
            (forward-line 1))
          (setq adjacent-start (point))
          (goto-char adjacent-end)
          (when (and (bolp) (> (point) adjacent-start))
            (forward-line -1))
          (while (and (looking-at-p "[ \t]*$")
                      (> (point) adjacent-start))
            (forward-line -1))
          (setq adjacent-end (line-end-position))
          (when (or (>= adjacent-start adjacent-end)
                    (and (not up) (not (= adjacent-start (if up
                                                (save-excursion
                                                  (goto-char current-start)
                                                  (forward-line -1)
                                                  (while (and (looking-at-p "[ \t]*$")
                                              ...
                                              (save-excursion
                                                ...
                                                (while (and (looking-at-p "[ \t]*$")
                                                  ...
                                                  (forward-line 1))
                                                (point)))))))
            (user-error "Adjacent Markdown block is not a paragraph"))
          (goto-char adjacent-start)
          (when (or (markdown-heading-at-point)
                    (markdown-list-item-at-point-p)
                    (markdown-table-at-point-p)
                    ...
                    (get-text-property (line-beginning-position)
                                       'markdown-metadata-markup))
            (user-error "Adjacent Markdown block is not a paragraph"))
          (dolist (bounds (list (cons current-start current-end)
                                (cons adjacent-start adjacent-end)))
            (goto-char (car bounds))
            (while (and (not invalid) (<= (point) (cdr bounds)))
              (when (or (markdown-heading-at-point)
                        (markdown-list-item-at-point-p)
                        (markdown-table-at-point-p)
                        (markdown-code-block-at-point-p)
                        (looking-at-p (concat "[ \t]*" markdown-regex-html-tag))
                        (markdown-line-is-reference-definition-p)
                        (markdown-text-property-at-point 'markdown-blockquote)
                        (markdown-text-property-at-point 'markdown-hr)
                        (markdown-text-property-at-point 'markdown-yaml-metadata-begin)
                        (markdown-text-property-at-point 'markdown-yaml-metadata-end)
                        (markdown-text-property-at-point 'markdown-yaml-metadata-section)
                        (markdown-text-property-at-point 'markdown-metadata-key)
                        (markdown-text-property-at-point 'markdown-metadata-markup)
                        (markdown-text-property-at-point 'markdown-metadata-value)
                        (and (looking-at-p markdown-regex-declarative-metadata)
                             (markdown-metadata-line-p
                              (point) markdown-regex-declarative-metadata))
                        (and (looking-at-p markdown-regex-pandoc-metadata)
                             (markdown-metadata-line-p
                              (point) markdown-regex-pandoc-metadata)))
                (setq invalid t))
              (forward-line 1)))
          (when invalid
            (user-error "Not an ordinary Markdown paragraph"))
          (goto-char adjacent-start)
          (setq adjacent-heading
                (when (markdown-back-to-heading-over-code-block nil t)
                  (point)))
          (unless (equal current-heading adjacent-heading)
            (user-error "Adjacent Markdown paragraph is in another section")))))
     (t
      (user-error "Not in a Markdown buffer")))
    (let* ((first-start (if up adjacent-start current-start))
           (first-end (if up adjacent-end current-end))
           (second-start (if up current-start adjacent-start))
           (second-end (if up current-end adjacent-end))
           (adjacent-length (- adjacent-end adjacent-start))
           (separator-length (- second-start first-end))
           (point-offset (- (point) current-start))
           (selection-active (region-active-p))
           (mark-offset (and selection-active (- (mark) current-start))))
      (unless (and (<= (point-min) first-start)
                   ...
                   (not (string-match-p
                         ...
                         (buffer-substring-no-properties
                          first-end second-start))))
        (user-error "Markdown paragraphs are not adjacent"))
      (when selection-active
        (unless (and (<= current-start (point))
                     (<= (point) current-end)
                     (<= current-start (mark))
                     (<= (mark) current-end))
          (user-error "Selection spans more than one Markdown paragraph")))
      (transpose-regions first-start first-end second-start second-end t)
      (goto-char (+ (if up
                          first-start
                        (+ first-start adjacent-length separator-length))
                    point-offset))
      (when selection-active
        (set-mark (+ (if up
                         first-start
...
(defun my/markdown-move-up ()
  "Move the Markdown structure at point up."
  (interactive)
  (cond
   ((eq major-mode 'markdown-ts-mode)
    (let ((node (treesit-node-at (point) 'markdown)))
      (cond
       ...
       ((and node (treesit-parent-until node "\\`list_item\\'" t))
        (markdown-ts-move-subtree-up))
       ((outline-on-heading-p)
        (markdown-ts-move-subtree-up))
       ((and node (treesit-parent-until node "\\`paragraph\\'" t))
        (my/markdown-swap-paragraphs t))
       (t
        (user-error "Not at a movable Markdown structure")))))
   ((derived-mode-p 'markdown-mode)
    (cond
     ((markdown-table-at-point-p)
...
(defun my/markdown-move-down ()
  "Move the Markdown structure at point down."
  (interactive)
  (cond
   ((eq major-mode 'markdown-ts-mode)
    (let ((node (treesit-node-at (point) 'markdown)))
      (cond
       ...
       ((outline-on-heading-p)
        (markdown-ts-move-subtree-down))
       ((and node (treesit-parent-until node "\\`paragraph\\'" t))
        (my/markdown-swap-paragraphs nil))
       (t
        (user-error "Not at a movable Markdown structure")))))
   ((derived-mode-p 'markdown-mode)
    (cond
...
(dolist
    (binding
     '(("," . my/select-buffer)
       ...
       ("f p" . my/find-file-recursive-configured)
