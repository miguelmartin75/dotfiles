...
(ert-deftest my/markdown-parity-insert-list-editing ()
  (dolist (mode '(markdown-ts-mode markdown-mode))
    ...
    (with-temp-buffer
      ...
      (let (called)
        ...
        (should called)))
    (with-temp-buffer
      (let* ((indentation (if (eq mode 'markdown-mode) "    " "  "))
             (text (concat "- parent\n" indentation "- child\n"))
             (expected (concat "- parent\n" indentation "- child\n- \n")))
        (insert text)
        (funcall mode)
        (font-lock-ensure)
        (goto-char (point-min))
  ...
  (with-temp-buffer
    ...
    (evil-insert-state)
    (markdown-ts-in-table-mode 1)
    (should (eq (key-binding (kbd "TAB")) #'markdown-ts-table-next-cell))
    (should (eq (key-binding (kbd "<tab>")) #'markdown-ts-table-next-cell))
    (should (eq (key-binding (kbd "S-TAB"))
...
(ert-deftest my/markdown-parity-access-and-binding-scope ()
  (let (normal-tab normal-control-i visual-tab visual-control-i)
    (with-temp-buffer
      (evil-normal-state)
      (setq normal-tab (key-binding (kbd "TAB"))
            normal-control-i (key-binding (kbd "C-i")))
      (evil-visual-state)
      (setq visual-tab (key-binding (kbd "TAB"))
            visual-control-i (key-binding (kbd "C-i"))))
    (dolist (mode '(markdown-ts-mode markdown-mode))
      (with-temp-buffer
        (funcall mode)
        ...
        (dolist (binding `((evil-normal-state ,normal-tab ,normal-control-i)
                           (evil-visual-state ,visual-tab ,visual-control-i)))
          (funcall (nth 0 binding))
          (should (eq (key-binding (kbd "TAB")) (nth 1 binding)))
          (should (eq (key-binding (kbd "C-i")) (nth 2 binding)))))
      (with-temp-buffer
        (insert "# first\nbody\n## child\nchild body\n# second\nsecond body\n")
        (funcall mode)
        (evil-normal-state)
