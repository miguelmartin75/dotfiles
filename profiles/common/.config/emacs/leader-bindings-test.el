...
(ert-deftest my/leader-bindings-completion-facades-stay-stable ()
  ...
  (dolist (state '(evil-normal-state evil-visual-state))
    ...
    (dolist (binding '(("SPC ," . my/select-buffer)
                       ...
                       ("SPC b B" . my/search-buffer-menu)
                       ("SPC s l" . my/search-line)
                       ("SPC s L" . my/search-line-menu)
                       ("SPC s b" . my/search-lines)
                       ("SPC s B" . my/search-lines-menu)
                       ("SPC s g" . my/search-ripgrep)
                       ("SPC s G" . my/search-ripgrep-menu)
                       ("SPC s w" . my/search-ripgrep-region-or-symbol)
                       ("SPC s W" . my/search-ripgrep-menu)
      ...
      (should (eq (key-binding (kbd (car binding)))
                  (cdr binding)))))
  (dolist (entry '(("b" "B" . "buffer options menu")
                   ("s" "L" . "line search options menu")
                   ("s" "B" . "multi-buffer line search options menu")
                   ("s" "G" . "ripgrep options menu")
                   ("s" "W" . "ripgrep options menu")))
    (let ((bindings (which-key--get-keymap-bindings
                     (keymap-lookup my/leader-map (car entry)))))
      (should (equal (assoc (nth 1 entry) bindings)
                     (cons (nth 1 entry) (cddr entry)))))))
